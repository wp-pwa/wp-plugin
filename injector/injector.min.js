!(function(n, a, e) {
  var t = /ip(hone|od).*?OS (?![1-8]_|X)/i,
    r = /ipad.*?OS (?![1-8]_|X)/i,
    o = /android.+chrome\/(?![123]\d\.)(.+mobile)/i,
    i = /android.+chrome\/(?![123]\d\.)(?!.+mobile)/i;
  (a["wp-pwa"].ssr_server = a["wp-pwa"].ssr_server.replace(/\/$/g, "") + "/"),
    (a["wp-pwa"].static_server =
      a["wp-pwa"].static_server.replace(/\/$/g, "") + "/");
  var s,
    p,
    c,
    d = function(e, t, r) {
    ...
    w = function(e) {
      for (
        var t = e + "=", r = decodeURIComponent(n.cookie).split(";"), a = 0;
        a < r.length;
        a++
      ) {
  ...
  if (w("wppwaClassicVersion"))
    ...
    e &&
    ((c = e.userAgent), t.test(c) || o.test(c))
  ) {
    a.stop();
    n.write(
      unescape(
    ...
    var l =
      ...
      a["wp-pwa"].id +
      "&static=" +
      encodeURIComponent(a["wp-pwa"].static_server) +
      "&perPage=" +
      a["wp-pwa"].per_page +
      "&device=" +
      ((p = e && e.userAgent), r.test(p) || i.test(p) ? "tablet" : "mobile") +
      "&initialUrl=" +
      encodeURIComponent(
        a["wp-pwa"].initial_url || a.location.origin + a.location.pathname
      );
    a["wp-pwa"].page && (l += "&page=" + a["wp-pwa"].page);
    var g = function(e, t) {
      ...
      r.open("POST", "https://api.rollbar.com/api/1/item/", !0),
        r.send(
          JSON.stringify({
            ...
            data: {
              ...
              body: {
                message: {
                  body: 'Injector "' + t + '" on: ' + a.location.href,
                  error: t,
                },
              },
            },
        ...
        console.error('Injector "' + t + '" on: ' + a.location.href);
    };
    ((s = new XMLHttpRequest()).timeout = 3e4),
        ...
        4 === s.readyState &&
          (200 === s.status
            ? ((e = s.responseText),
              ...
              t.close())
            : (d("wppwaInjectorFailed", "true", 3), a.location.reload(!0)));
      }),
      ...
      s.open("GET", a["wp-pwa"].ssr_server + l, !0),
      s.send();
  }
